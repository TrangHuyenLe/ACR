...
router.get(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s/:jobId/results',
...
router.get(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s/:jobId/d/results',
  JwtFilter(cfg.SECRET),
  InjectTermId(),
  ExceptionWrapper(async (req, resp) => {
    const query = modify(req.query, [m.pick(['q'], { dropFalsy: true })]);
...
router.get(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s',
  JwtFilter(cfg.SECRET),
  InjectTermId(),
  ExceptionWrapper(async (req, resp) => {
    const query = modify(req.query, [m.pick(['q'], { dropFalsy: true })]);
...
router.post(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s/:jobId/retry',
  JwtFilter(cfg.SECRET),
  InjectTermId(),
  ExceptionWrapper(async (req, resp) => {
    const accountId = req.__accountId;
    ...
    await mongoConnectionPool
      ...
      .updateOne(
        ...
        { $set: { status: JobStatus.READY, timeToStart: Date.now() } }
      );
...
router.put(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s/:jobId/cancel',
  JwtFilter(cfg.SECRET),
  InjectTermId(),
  ExceptionWrapper(async (req, resp) => {
    const accountId = req.__accountId;
...
router.delete(
  '/api/accounts/current/term-ids/:termId/v1/dkhptd-s/:jobId',
  JwtFilter(cfg.SECRET),
  InjectTermId(),
  ExceptionWrapper(async (req, resp) => {
    const accountId = req.__accountId;
