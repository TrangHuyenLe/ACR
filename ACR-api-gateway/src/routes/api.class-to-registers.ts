...
import { toBuffer, toNormalizedString, toSafeInt } from '../utils';
...
router.post(
  '/api/class-to-registers/file',
  JwtFilter(cfg.SECRET),
  IsAdminFilter(),
  multer({ limits: { fileSize: 5 * 1000 * 1000 /* 5mb */ } }).single('file'),
  ExceptionWrapper(async (req, resp) => {
    const file = req.file;
    logger.info(`Received uploaded TKB xlsx length ${file.buffer.length}`);
    rabbitmqConnectionPool
      .getChannel()
      .sendToQueue(QueueName.PARSE_TKB_XLSX, toBuffer(file.buffer));
...
router.get(
  '/api/class-to-registers/class-ids',
  ExceptionWrapper(async (req, resp) => {
...
router.get(
  '/api/class-to-registers/class-ids/start-withs',
  ExceptionWrapper(async (req, resp) => {
    const classIds = toNormalizedString(req.query.classIds)
      .split(',')
...
router.get(
  '/api/class-to-registers/class-ids/:classId',
  ExceptionWrapper(async (req, resp) => {
